...
if [ "$SKIPTESTS" = "skipTests" ]; then
   mvn package -DskipTests -Pcassandra,inmemory,jpa,elasticsearch,lucene,with-assembly,with-jetm -T1C
else
   mvn package -Pcassandra,inmemory,jpa,elasticsearch,lucene,with-assembly,with-jetm -T1C
fi
